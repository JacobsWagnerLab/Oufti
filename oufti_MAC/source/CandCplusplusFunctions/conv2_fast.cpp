...
{ 
  ...
  if (!strcmp(modep,default_mode)){ // Full convolution
    ...
    OUTPUT = mxCreateNumericArray(ndims,outputdims, mxSINGLE_CLASS, mxREAL);
	...
	imagep = imagep_base + (image_size.width*image_size.height);

	// set pointer offset for output
        outputp = outputp_base + (output_size.width*output_size.height);
    ...
    status =  ippiConvGetBufferSize(image_size,kernel_size,ipp16s,1,funCfgValid,&bufSizeFull);
    //if ( status1 != ippStsNoErr ) return status1;
    bufSizeMax = IPP_MAX(bufSizeFull, bufSizeValid); // get max buffer size
    pBuffer = ippsMalloc_8u( bufSizeMax );
	retval = ippiConv_32f_C1R(imagep,sizeof(float)*image_size.width,image_size,kernelp,sizeof(float)*kernel_size.width,kernel_size,outputp,sizeof(float)*output_size.width,funCfgFull,pBuffer);
...
//#pragma omp parallel num_threads(number_threads)
   ...
   // #pragma omp for
	  ...
	  retval = ippiConv_32f_C1R(imagep,sizeof(float)*image_size.width,image_size,kernelp,sizeof(float)*kernel_size.width,kernel_size,outputp,sizeof(float)*output_size.width,funCfgValid,pBuffer);
