...
handles.imageViewer = figure('Position',[100 scrSize(4)/7 scrSize(4)+10 scrSize(4)-340],'units','pixels',...
...
%--------------------------------------------  Data Panel ------------------------------------------------------------
handles.dataPanel = uipanel('units','pixels','Title','Load Data','pos',[scrSize(4)-250+30 (scrSize(4)-500) 160 100]);  
...
handles.currentframe = uicontrol(handles.imagePanel,'units','pixels','Position',[scrSize(4)-250+80 (scrSize(4)-730) 50 20],...
          'Style','edit','HorizontalAlignment','left','HitTest','off','SelectionHighlight','off');
%------------------------------------------------------------------------------------------------------------------------------
...
function selectclick(hObject, EventType)%#ok<INUSD>
    ...
    elseif get(handles.addSpot,'value') == 1
        ...
        spotStructure.rsquared = gof.adjrsquare;
        spotStructure.confidenceInterval_b_h_w_x_y = confidenceInterval;
        if isfield(cellStructure,'spots') && size(cellStructure.spots,2) >= 1
            try
                cellStructure.spots.rsquared = cat(2,cellStructure.spots.rsquared,spotStructure.rsquared);
                ...
                cellStructure.spots.confidenceInterval_b_h_w_x_y = cat(2,cellStructure.spots.confidenceInterval_b_h_w_x_y,spotStructure.confidenceInterval_b_h_w_x_y);
    ...
    elseif get(handles.removeSpot,'value') == 1
        ...
        if size(indexX,1) < size(indexY,1)
            ...
            cellStructure.spots.confidenceInterval_b_h_w_x_y(indexX) = [];
        ...
        elseif size(indexX,1) > size(indexY,1)
            ...
            cellStructure.spots.confidenceInterval_b_h_w_x_y(indexY) = [];
            ...
            if isempty(box)
                for ii = 1:length(cellList.meshData{frame})
                    if isfield(cellList.meshData{frame}{ii},'mesh')
                     ...
                     text(cntx,cnty,num2str(cellList.cellId{frame}(ii)),'hittest','off','Clipping','on','HorizontalAlignment','center','FontSize',9,'color','y');
                ...
                if ~isempty(spotStructure.w) 
                     line([xValue*.021,yValue*.05],[xValue*.021,xValue*0.021],'LineStyle','-','LineWidth',4,'Color',[0 0.5 0]);
                     text(xValue*.07,xValue*0.021,'R squared','Color',[0 0.5 0],'FontSize',12);
                     line([xValue*.021,yValue*.05],[xValue*.05,xValue*0.05],'LineStyle','-','LineWidth',4,'Color',[0 0.5 0.5]);
                     text(xValue*.07,xValue*.05,'Overall Intensity','Color',[0 0.5 0.5],'FontSize',12);
                     ...
                     for k = 1:length(spotStructure.w)
                        ...
                        try
                            text(xValue*0.45,k*3,num2str(spotStructure.rsquared(k)),'Color',[0 0.5 0],'FontSize',10);
                        catch
                            text(xValue*0.45,k*3,num2str(spotStructure.rmse(k)),'Color',[0 0.5 0],'FontSize',10);
                        end
                        text(xValue*0.6,k*3,num2str(spotStructure.magnitude(k)),'Color',[0 0.5 0.5],'FontSize',10);
                        text(xValue*0.75,k*3,num2str(spotStructure.w(k)),'Color',[0.7 0 0],'FontSize',10); 
                        text(xValue*0.85,k*3,num2str(spotStructure.h(k)),'Color',[1 0.5 0.5],'FontSize',10);
                     end
