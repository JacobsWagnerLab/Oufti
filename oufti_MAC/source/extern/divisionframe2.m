function [cellIdContainer frameIdContainer] = divisionframe2(cellList,threshValue,flag)

divfrm = [];
nonEmptyFrameIndex = find(~cellfun(@isempty,cellList.meshData) > 0);
[~,cellId] = oufti_getFrame(nonEmptyFrameIndex(end),cellList);
for celln = cellId
    cellprf = [];
    for frame=1:oufti_getLengthOfCellList(cellList)
        ...
        if oufti_doesCellStructureHaveMesh(celln, frame, cellList) ...
                ...
                && (length(tmpCellStructure.divisions) == 1 || tmpCellStructure.birthframe ~= 1)
