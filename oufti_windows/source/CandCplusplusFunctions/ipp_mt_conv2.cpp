...
#define KERNEL          prhs[1] // The kernel: i x j matrix of single precision floating point numbers. // String: either "full" or "valid". Unless string is "full", code will default to "valid".
...
{
  int status,buflen,num_images,num_dims;
  float *kernelp, *imagep, *outputp, *imagep_base;
  ...
  IppStatus retval, status1;
  IppiSize output_size, kernel_size, image_size;
  int bufSizeFull,bufSizeValid,bufSizeMax,divisor = 1;
  Ipp8u *pBuffer;
 ...
 // imagedims = mxGetDimensions(IMAGE);
  ...
  image_size.width = mxGetM(IMAGE);
  image_size.height = mxGetN(IMAGE);
  /*if (num_dims == 2){
    num_images = 1;}
  else{
  ...
  kernel_size.width  =  mxGetM(KERNEL);
  kernel_size.height = mxGetN(KERNEL);
  ...
  // Decide Full or valid. Default is valid 
    ...
    //OUTPUT = mxCreateNumericArray(ndims,outputdims, mxDOUBLE_CLASS, mxREAL);
    OUTPUT = mxCreateNumericMatrix(output_size.width,output_size.height, mxSINGLE_CLASS, mxREAL);
    ...
    //status1 = ippiConvGetBufferSize(image_size,kernel_size,ipp16s,1,funCfgFull,&bufSizeFull);
    //if ( status1 != ippStsNoErr ) return status1;
    //status1 =  ippiConvGetBufferSize(image_size,kernel_size,ipp16s,1,funCfgValid,&bufSizeValid);
    //if ( status1 != ippStsNoErr ) return status1;
    bufSizeMax = IPP_MAX(bufSizeFull, bufSizeValid); // get max buffer size
    pBuffer = ippsMalloc_8u( bufSizeMax );
	retval = ippiConv_32f_C1R(imagep,sizeof(float)*image_size.width,image_size,kernelp,sizeof(float)*kernel_size.width,kernel_size,outputp,sizeof(Ipp32f)*output_size.width,funCfgFull,pBuffer);
