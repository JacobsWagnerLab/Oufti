...
function m = serialize_cell_heterogenous(v)
    contents = cellfun(@serializeData,v,'UniformOutput',false);
    ...
    elseif isempty(v)
        ...
        elseif (size1+size2 == 0) & (dims == 2) %#ok<AND2>
            ...
            elseif length(unique(cellfun(@class,v(:),'UniformOutput',false))) == 1
                ...
                m = [uint8(38); serializeData(class(v{1})); ndims(v); typecast(uint32(size(v)),'uint8').'];
            else
