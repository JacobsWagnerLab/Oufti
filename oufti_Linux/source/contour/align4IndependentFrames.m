...
function [pcc,ftq] = align4IndependentFrames(im,mpx,mpy,ngmap,pcc,p,roiBox,thres,celldata)
%warning off all
...
mesh = model2mesh(pcc,p.fmeshstep,p.meshTolerance,p.meshWidth);
if length(mesh)>4
   pcc = fliplr([mesh(:,1:2);flipud(mesh(:,3:4))]);
else
   pcc = [];
end
...
if p.moveall>0
   if a>1
    ...
    xm = xCell-mean(xCell); ym = yCell-mean(yCell);
    MF = mean(-Fy.*xm+Fx.*ym); MI = sum(xm.^2+ym.^2);
    Fmrx =  ym*MF/MI; Fmry = -xm*MF/MI;
    mfx = mean(Fx); mfy = mean(Fy);
