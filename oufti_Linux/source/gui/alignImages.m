function [shiftX,shiftY] = alignImages(imageData)
%-----------------------------------------------------------------------------------------
%-----------------------------------------------------------------------------------------
%function [shiftX,shiftY] = alignImages(imageData)
%oufti.v1.0.7
...
%=================================================================================
%**********output********:
%shiftX: container having the x-values for image shift
%shiftY: container having hte y-values for image shift
%**********Input********:
%imageData:  container having all the raw image data
%-----------------------------------------------------------------------------------------

shiftX = 0;
shiftY = 0;
numImages = size(imageData,3);
time1 = clock;
for ii = 1:numImages-1
    
     [output,~] = dftregistration(fft2(imageData(:,:,ii)),fft2(imageData(:,:,ii+1)),50);
     
% % % % Cross-correlate image 1 and image 2 to find offset data   
% % % c = normxcorr2(imageData(:,:,ii),imageData(:,:,ii+1));
% % % %%-----------------------------------------------------------------------------------------
% % % % offset found by correlation
% % % [max_c, imax] = max(abs(c(:)));
% % % [ypeak, xpeak] = ind2sub(size(c),imax(1));
% % % corr_offset = [(xpeak-size(imageData(:,:,ii),2)) (ypeak-size(imageData(:,:,ii),1))];
% % % %%-----------------------------------------------------------------------------------------
% % % shiftX = [shiftX; corr_offset(2)];%#okgrow
% % % shiftY = [shiftY; corr_offset(1)];%#okgrow
   shifts = output(3:4);
   shiftX = [shiftX; -shifts(2)];%#okgrow
   shiftY = [shiftY; -shifts(1)];%#okgrow
disp(['frame = ' num2str(ii+1) ', shift ' num2str(-shifts(2)) ',' num2str(-shifts(1)) ' pixels'])  
end
time2 = clock;
